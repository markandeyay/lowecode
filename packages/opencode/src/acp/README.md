...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for LOWECODE.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal LOWECODE sessions
...
lowecode acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 lowecode acp
```
...
- **Terminal Integration**: Full terminal support via LOWECODE's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | lowecode acp
```
...
ACP sessions map cleanly to LOWECODE's internal session model:
