...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run lowecode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for LOWECODE session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found LOWECODE session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["lowecode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting LOWECODE...")
        UI.println()

        // Launch LOWECODE TUI with session ID if available
        const { spawn } = await import("child_process")
        const lowecodeArgs = sessionId ? ["-s", sessionId] : []
        const lowecodeProcess = spawn("lowecode", lowecodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          lowecodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`lowecode exited with code ${code}`))
          })
          lowecodeProcess.on("error", reject)
        })
      },
    })
  },
