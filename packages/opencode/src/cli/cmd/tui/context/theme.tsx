...
import chipotle from "./theme/chipotle.json" with { type: "json" }
import lowecodeBlue from "./theme/lowecode-blue.json" with { type: "json" }
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: (config.theme ?? kv.get("theme", "lowecode-blue")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes["lowecode-blue"], store.mode)
    })
