...
const AGENT_USERNAME = "lowecode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/lowecode.yml"
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/lc summarize` to see the agent in action",
                "",
                "   Learn more about the GitHub agent in the LOWECODE README",
              ].join("\n"),
            )
          }
...
jobs:
  lowecode:
    if: |
      ...
      contains(github.event.comment.body, ' /lowecode') ||
      startsWith(github.event.comment.body, '/lowecode')
    ...
    steps:
      ...
      - name: Run LOWECODE
        uses: markandeyay/lowecode/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      try {
        ...
        console.log("LOWECODE session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/lowecode,/lc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to LOWECODE...")
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `lowecode/${type}-${hex}-${timestamp}`
        }
        return `lowecode/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const shareUrl = shareId ? `[LOWECODE session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the LOWECODE infrastructure after your response",
