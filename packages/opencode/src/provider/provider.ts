...
export namespace Provider {
  ...
  const BUNDLED_PROVIDERS: Record<string, (options: any) => SDK> = {
    ...
    "lowes-mylow": () =>
      createOpenAICompatible({
        name: "lowes-mylow",
        baseURL: process.env.LOWES_MYLOW_BASE_URL || "http://localhost:3000/v1",
        apiKey: process.env.LOWES_MYLOW_API_KEY || "lowecode-local",
      }),
  }
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async "lowes-mylow"() {
      const baseURL = process.env.LOWES_MYLOW_BASE_URL || "http://localhost:3000/v1"
      log.info("LOWECODE provider: lowes-mylow")
      log.info("LOWECODE model: mylow-1")
      log.info("LOWECODE base URL: " + baseURL)
      return {
        autoload: true,
        options: {
          apiKey: process.env.LOWES_MYLOW_API_KEY || "lowecode-local",
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `lowecode auth cloudflare-ai-gateway`.",
        )
      }
  ...
  const state = Instance.state(async () => {
    ...
    const lowesBaseURL = process.env.LOWES_MYLOW_BASE_URL || "http://localhost:3000/v1"

    const bots = [
      ["lowes-mylow", "Lowe's Mylow", "mylow-1", "Mylow 1"],
    ] as const

    for (const bot of bots) {
      const provider = ProviderID.make(bot[0])
      const model = ModelID.make(bot[2])
    ...
    const allowOthers = process.env.LOWECODE_ALLOW_OTHER_PROVIDERS === "1"
    if (!allowOthers) {
      for (const id of Object.keys(providers)) {
        if (id === "lowes-mylow") continue
        if (enabled && enabled.has(ProviderID.make(id))) continue
        delete providers[id]
      }
    }
